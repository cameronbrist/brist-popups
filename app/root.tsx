...
async function loadCriticalData({request, context}: Route.LoaderArgs) {
  ...
  if (!resolved) {
    throw new Response('No pop-up store is live at this address.', {
      status: 404,
    });
  }
  ...
  return {
    ...
    drop: resolved.drop,
    themeCss: themeToCss(resolved.popup.theme),
  };
}
...
export function Layout({children}: {children?: React.ReactNode}) {
  ...
  const fontsHref = data?.popup.theme.fonts.href;

  return (
    <html lang="en" data-layout={data?.popup.layout}>
      <head>
        <meta charSet="utf-8" />
...
export function ErrorBoundary() {
  ...
  return (
    <div className="route-error">
      <h1>{errorStatus === 404 ? 'Nothing here' : 'Something went wrong'}</h1>
